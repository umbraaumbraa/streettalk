...
const renderV = () => {
    ...
    if (item.type.includes('video')) {
        const v = document.createElement('video'); v.src = item.data; 
...
const renderPre = () => {
    ...
    pending.forEach((m, i) => {
        ...
        d.className = 'pre-item';
        d.innerHTML = `
            ${m.type.includes('video') ? `<video src="${m.data}"></video>` : `<img src="${m.data}">`}
            <button class="pre-cancel" onclick="window.remPre(${i})">✕</button>
        `;
        g.appendChild(d);
    });
};
...
const renderThread = (data, container, path = "shouts") => {
    ...
    keys.forEach(key => {
        ...
        postDiv.innerHTML = `<div class="post"><img src="${p.userAvatar || ''}" class="avatar"><div style="flex:1"><div class="post-info"><span style="font-weight:700">${p.user}</span><span class="post-time">${timeStr}</span></div><div style="color:var(--text); line-height: 1.5; font-size: 0.95rem;">${p.content}</div>${mediaHtml}<div class="post-actions"><button class="action-btn upvote-btn ${hasVoted ? 'active' : ''}" onclick="window.toggleUpvote('${currentPath}')">▲ ${voteCount}</button><button class="action-btn" onclick="window.showReplyBox('${key}', '${currentPath}')">💬 Reply</button></div><div id="reply-container-${key}" class="hidden"></div></div></div><div id="children-${key}" class="reply-thread"></div>`;
        ...
        if (p.media) p.media.forEach((m, i) => {
            ...
            if (cell) {
                cell.onclick = (e) => {
                    ...
                    const isPlayBtn = e.target.classList.contains('play-overlay');
                    if (isPlayBtn && video) { e.stopPropagation(); cell.classList.add('playing'); video.controls = true; video.loop = false; video.play(); video.onended = () => { cell.classList.remove('playing'); video.controls = false; }; }
...
const refreshFeed = () => { const stream = document.getElementById('feedStream'); stream.innerHTML = ""; globalMediaMap = {}; if (lastSnapData) renderThread(lastSnapData, stream); };
onValue(query(ref(db, 'shouts'), limitToLast(50)), (snap) => { lastSnapData = snap.val(); refreshFeed(); });
...
window.showReplyBox = (key, path) => {
    ...
    if(isHidden) { container.classList.remove('hidden'); container.innerHTML = `<div class="reply-composer" style="margin-top:15px;"><textarea id="reply-input-${key}" placeholder="Add to the thread..."></textarea><div style="text-align:right"><button class="post-action-btn" onclick="window.submitReply('${key}', '${path}')" style="padding: 6px 20px; font-size: 0.8rem;">Reply</button></div></div>`; container.querySelector('textarea').focus(); }
};

window.submitReply = (key, path) => { const input = document.getElementById(`reply-input-${key}`); if (!input.value.trim()) return; push(ref(db, `${path}/replies`), { userId: user.id, user: user.name, userAvatar: user.avatar, content: input.value, timestamp: Date.now() }); };
window.toggleUpvote = (path) => { runTransaction(ref(db, `${path}/votes`), (v) => { v = v || {}; if (v[user.id]) delete v[user.id]; else v[user.id] = true; return v; }); };
...
document.getElementById('openSettings').onclick = () => { document.getElementById('setName').value = user.name; document.getElementById('settingsAvatarPreview').innerHTML = `<img src="${user.avatar || ''}" class="avatar" style="width:100px; height:100px; border:2px solid var(--accent);">`; document.getElementById('settingsModal').classList.remove('hidden'); };
document.getElementById('closeSettings').onclick = () => document.getElementById('settingsModal').classList.add('hidden');
document.getElementById('saveSettings').onclick = () => { user.name = document.getElementById('setName').value || "Stranger"; localStorage.setItem('ST_NAME', user.name); localStorage.setItem('ST_AVATAR', user.avatar); location.reload(); };
document.getElementById('avatarUpload').onchange = (e) => { const r = new FileReader(); r.onload = (ev) => { user.avatar = ev.target.result; document.getElementById('settingsAvatarPreview').innerHTML = `<img src="${user.avatar}" class="avatar" style="width:100px; height:100px; border:2px solid var(--accent);">`; }; r.readAsDataURL(e.target.files[0]); };
...
const emojiPopup = document.getElementById('emojiContainer');
emojiBtn.onclick = (e) => { e.stopPropagation(); emojiPopup.classList.toggle('hidden'); };
document.addEventListener('click', (e) => { if (!emojiPopup.classList.contains('hidden')) { if (!emojiPopup.contains(e.target) && e.target !== emojiBtn) { emojiPopup.classList.add('hidden'); } } });
...
const sync = () => { const pic = `<img src="${user.avatar || ''}" class="avatar">`; document.getElementById('myProfilePic').innerHTML = pic; document.getElementById('inputAvatar').innerHTML = pic; };
sync();
